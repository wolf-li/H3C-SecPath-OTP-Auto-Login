...
*/
  ...
  */
  ...
  function readPassFile(file) {
      ...
      Version 1.0
      Dependencies:
          Read the file and return an array data.
      ***************************************************************************/
      var fso      = new ActiveXObject("Scripting.FileSystemObject");
      var passFile = fso.OpenTextFile(file);
      
      // The line-by-line reads are placed into a two-dimensional array
      var data = new Array();
      while (!passFile.AtEndOfStream) 
      { 
  ...
  function Main()
  {
    ...
    xsh.Session.Open(ssessionPath);
	xsh.Screen.Synchronous = true;

    // Calculate OTP token
    var totpObj = new TOTP();

    var otp = totpObj.getOTP(token);
    
    // Login in the server. Notice, this is using the H3C SecPath machine 
    xsh.Screen.Send('\n');
    xsh.Screen.WaitForString("2nd Password:");
    xsh.Screen.Send(otp + '\n');
    xsh.Screen.Send('0\n');
    xsh.Screen.Send(IP + '\n');
    xsh.Screen.Send("1\n");
